...
class AssignExternalIdToSubUC @Inject constructor(
    ...
    operator fun invoke(externalId: String): Flow<Resource<SubscribersWithGivenExternalIdDTO>> = flow {
        try {
            ...
            val assignExternalId = repository.assignExternalIdToSubscriber(externalId)
            emit(Resource.Success(assignExternalId))
