...
class TransactionalPushByExternalIdUC @Inject constructor(
    private val repository: TransactionalScreenRepository
) {
    operator fun invoke(externalId: String): Flow<Resource<MessageIdDTO>> = flow {
        try {
            emit(Resource.Loading())
            val sendTransactionalPush = repository.sendNotificationByExternalId(externalId)
